...
def create_as_mem_heatmap(
    ...
    """
    Create memory heatmap grouped by AS (Application System)
    
    Args:
        analysis_df: DataFrame with columns: as_name, server, timestamp, mem.usage.average
        ...
        sort_by: Sort option ("Суммарной нагрузке", "Средней нагрузке", "Мощности RAM", "Имени АС")
        sort_order: Sort order ("По убыванию" or "По возрастанию")
    
    Returns:
        Tuple of (figure, y_labels, x_labels, values_matrix, pivot_df)
    """
    if 'mem.usage.average' not in analysis_df.columns:
        raise ValueError("DataFrame must contain 'mem.usage.average' column")
    ...
    if sort_by == "Суммарной нагрузке":
        pivot_df = pivot_df.sort_values('total_load', ascending=(sort_order == "По убыванию"))
    elif sort_by == "Средней нагрузке":
        pivot_df = pivot_df.sort_values('avg_load', ascending=(sort_order == "По убыванию"))
    elif sort_by == "Мощности RAM":
        pivot_df['capacity_ram'] = pivot_df['server'].map(server_ram_capacity_map)
        pivot_df = pivot_df.sort_values('capacity_ram', ascending=(sort_order == "По убыванию"))
    else:  # "Имени АС"
        pivot_df = pivot_df.sort_values(['as_name', 'server'], ascending=(sort_order == "По убыванию"))
    ...
    for i, (_, row) in enumerate(pivot_df.iterrows()):
        ...
        for j, interval in enumerate(range(48)):
            ...
            if load_value <= 0:
                ...
                # Color categorization for memory
                if load_value < 25:
                    load_status = "🟢 Низкая"
                elif load_value < 80:
                    load_status = "🟡 Средняя"
                ...
                # Calculate absolute load in GB
                absolute_load_gb = (load_value / 100) * ram_capacity
                ...
                text = (f"<b>{as_name} | {server}</b><br>"
                       ...
                       f"🏷️ {load_status}<br>"
                       f"📈 Абс. нагрузка: {absolute_load_gb:.1f} GB")
    ...
    fig.update_layout(
        height=chart_height,
        title=dict(
            text=f"Нагрузка памяти по серверам в разрезе АС<br><sup>Сортировка: {sort_by}, {sort_order}</sup>",
...
def create_separate_as_heatmaps(
    ...
    """
    Create separate memory heatmaps for each AS
    ...
    for as_name, as_df in as_groups:
        try:
            fig, _, _, _, _ = create_as_mem_heatmap(
                as_df,
            ...
            fig.update_layout(
                title=dict(
                    text=f"АС: {as_name}<br>Нагрузка памяти по серверам",
                    font=dict(size=16),
                    x=0.5,
                    xanchor='center'
                )
